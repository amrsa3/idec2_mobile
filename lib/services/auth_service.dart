...
class AuthService {
    ...
    try {
      ...
      try {
        ...
        userJson = {
          ...
          'phone': user.phone,
          'email': user.email,
          'firstName': '', // firstName not available in UserModel
          'lastName': '', // lastName not available in UserModel
          'fullNameAr': '', // fullNameAr not available in UserModel
          ...
          'profilePictureUrl':
              '', // profilePictureUrl not available in UserModel
          'profilePicture': '', // profilePicture not available in UserModel
      ...
      try {
        ...
        _printDetailedError(
            'Storage operation failed', storageError, stackTrace,
            additionalData: {
      ...
      _printDetailedError('General storage error', generalError, stackTrace,
          additionalData: {
            'userId': user.id,
            'userPhone': user.phone,
          });
        ...
        if (responseData.containsKey('user') &&
          ...
          if (responseData.containsKey('tokens')) {
            ...
            if (tokensData != null) {
              if (accessToken.isEmpty &&
                  tokensData.containsKey('accessToken')) {
          ...
          if (accessToken.isNotEmpty) {
            try {
              ...
              _printDetailedError(
                  'Error storing user data in login', storageError, null,
                  additionalData: {
                    'userId': user.id,
                    'userPhone': user.phone,
  ...
  Future<AuthResponse> register(RegisterRequest request) async {
    try {
      ...
      if (!registrationAllowed) {
        ...
        return _createErrorResponse(
            status.message ?? 'Registration is not currently available');
      }
      ...
      debugPrint(
          'Registration API response: status=${response.statusCode}, data=${response.data}');
  ...
  Future<ApiResponse> requestOtp(String phoneNumber,
  ...
  Future<AuthResponse> verifyOtp(String phoneNumber, String otp) async {
    try {
      ...
      debugPrint(
          '🔍 [OTP_DEBUG] Response data type: ${response.data.runtimeType}');
      ...
      if (response.data is! Map<String, dynamic>) {
        throw Exception(
            'Response data is not a Map<String, dynamic>: ${response.data.runtimeType}');
      }

      final responseData = response.data as Map<String, dynamic>;
      debugPrint(
          '🔍 [OTP_DEBUG] Response data keys: ${responseData.keys.toList()}');

      // Use AuthResponse.fromJsonSafe to parse the response directly
      debugPrint(
          '🔍 [OTP_DEBUG] Attempting to parse AuthResponse with fromJsonSafe...');
      ...
      if (authResponse.user != null) {
        try {
          ...
          _printDetailedError(
              'Error storing user data in OTP verification', storageError, null,
              additionalData: {
      ...
      debugPrint(
          '🔍 [OTP_DEBUG] DioException caught: ${e.response?.statusCode} - ${e.message}');
      ...
      if (e.response?.statusCode == 200 && e.response?.data != null) {
        try {
          debugPrint(
              '🔍 [OTP_DEBUG] Processing 200 response from DioException...');
          ...
          if (authResponse.user != null) {
            try {
              final storageSuccess =
                  await _safeStoreUserData(authResponse.user!);
              if (!storageSuccess) {
                debugPrint(
                    '🔍 [OTP_DEBUG] Failed to store user data safely (from DioException)');
                // Continue without failing the entire operation
              }
            } catch (storageError) {
              _printDetailedError(
                  'Error storing user data in OTP verification (DioException)',
                  storageError,
                  null,
                  additionalData: {
                    'userId': authResponse.user?.id,
          ...
          debugPrint(
              '🔍 [OTP_DEBUG] OTP verification successful (from DioException)');
          return authResponse;
        } catch (parseError) {
          debugPrint(
              '🔍 [OTP_DEBUG] Error parsing successful OTP response: $parseError');
        }
      }
  ...
  Future<AuthResponse> refreshToken() async {
    try {
      ...
      debugPrint(
          'Token refresh DioException: ${e.response?.statusCode} - ${e.response?.data}');
  ...
  Future<PasswordResetResponse> resetPassword(
    ...
    try {
      ...
      if (e.response?.data != null &&
        ...
        final errorData = e.response!.data as Map<String, dynamic>;
        final errorMessage =
            errorData['message'] ?? 'حدث خطأ أثناء إعادة تعيين كلمة المرور';
        throw Exception(errorMessage);
      }
