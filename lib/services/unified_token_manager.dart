...
class UnifiedTokenManager {
  ...
  static UnifiedTokenManager get instance => _instance ??= UnifiedTokenManager._();
  ...
  Future<void> saveTokens({
    ...
    try {
      ...
      final refreshTokenExpiry = now.add(Duration(days: 30)); // 30 days for refresh token
      ...
      debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] ExpiresIn from server: $expiresIn seconds (${expiresIn / 86400} days)');
      debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] Calculated access token expiry: $accessTokenExpiry');
      debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] Calculated refresh token expiry: $refreshTokenExpiry');
      debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] Time until access token expires: ${accessTokenExpiry.difference(now).inDays} days, ${accessTokenExpiry.difference(now).inHours % 24} hours');
      
      // Save tokens securely
      await _storage.writeSecure(_accessTokenKey, accessToken);
      await _storage.writeSecure(_refreshTokenKey, refreshToken);
      await _storage.writeSecure(_tokenExpiryKey, accessTokenExpiry.toIso8601String());
      await _storage.writeSecure(_refreshTokenExpiryKey, refreshTokenExpiry.toIso8601String());
  ...
  Future<String?> getValidAccessToken() async {
    ...
    try {
      ...
      debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] Access token expired, attempting refresh');
      ...
      debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] Error getting valid access token: $e');
      return null;
    }
  }
  ...
  Future<bool> isAccessTokenValid() async {
    ...
    try {
      ...
      if (expiryString == null) {
        ...
        debugPrint('🔐 [TOKEN_DEBUG] Access token exists but no expiry - treating as invalid');
        return false;
      }
      ...
      debugPrint('🔐 [TOKEN_DEBUG] Buffer time: ${bufferTime.inMinutes} minutes');
      debugPrint('🔐 [TOKEN_DEBUG] Effective expiry (with buffer): $effectiveExpiry');
      debugPrint('🔐 [TOKEN_DEBUG] Time until actual expiry: ${timeUntilExpiry.inDays} days, ${timeUntilExpiry.inHours % 24} hours, ${timeUntilExpiry.inMinutes % 60} minutes');
      debugPrint('🔐 [TOKEN_DEBUG] Time until effective expiry: ${timeUntilEffectiveExpiry.inDays} days, ${timeUntilEffectiveExpiry.inHours % 24} hours, ${timeUntilEffectiveExpiry.inMinutes % 60} minutes');
      debugPrint('🔐 [TOKEN_DEBUG] Is token valid (with buffer): $isValid');
      debugPrint('🔐 [TOKEN_DEBUG] Access token length: ${accessToken.length} characters');
      debugPrint('🔐 [TOKEN_DEBUG] Access token prefix: ${accessToken.substring(0, accessToken.length > 20 ? 20 : accessToken.length)}...');
      
      if (!isValid) {
        debugPrint('🔐 [TOKEN_DEBUG] ❌ Token is invalid or expired');
        if (timeUntilExpiry.isNegative) {
          debugPrint('🔐 [TOKEN_DEBUG] Token has already expired ${timeUntilExpiry.abs().inMinutes} minutes ago');
        } else {
          debugPrint('🔐 [TOKEN_DEBUG] Token expires in ${timeUntilExpiry.inMinutes} minutes but buffer makes it invalid');
        }
      } else {
  ...
  Future<bool> hasValidRefreshToken() async {
    ...
    try {
      ...
      debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] Error checking refresh token validity: $e');
      return false;
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await saveTokens(
          ...
          expiresIn: data['expiresIn'] ?? 2592000, // Default 30 days (30 * 24 * 60 * 60 = 2592000 seconds)
        ...
        debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] Token refresh failed: ${response.statusCode}');
  ...
  Future<void> clearTokens() async {
    ...
    try {
      ...
      debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] All tokens cleared');
  ...
  Future<void> _updateLastActivity() async {
    try {
      await _storage.writeSecure(_lastActivityKey, DateTime.now().toIso8601String());
    } catch (e) {
  ...
  void _startSessionMonitoring() {
    ...
    _sessionTimer = Timer.periodic(Duration(minutes: 30), (timer) async { // تغيير من 5 دقائق إلى 30 دقيقة
      try {
        ...
        if (expiryString != null) {
          ...
          if (timeUntilExpiry.inHours <= 6 && timeUntilExpiry.inHours > 0) { // تغيير من 10 دقائق إلى 6 ساعات
            debugPrint('🔐 [UNIFIED_TOKEN_MANAGER] Token expires soon, refreshing proactively');
