...
class ApiEndpointTester {
  ...
  Future<void> testAuthEndpoints() async {
    ...
    // Test refresh token
    try {
      final response = await makeRequest('POST', '/api/v1/auth/refresh', requiresAuth: true);
      if (response.statusCode == 200) {
  ...
  Future<void> testOtpEndpoints() async {
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/auth/otp/send', body: otpData);
      if (response.statusCode == 200) {
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/auth/otp/verify', body: verifyData);
  ...
  Future<void> testUserEndpoints() async {
    ...
    try {
      final response = await makeRequest('GET', '/api/v1/users/profile', requiresAuth: true);
    ...
    try {
      final response = await makeRequest('PUT', '/api/v1/users/profile', body: updateData, requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
  ...
  Future<void> testFileEndpoints() async {
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/files/upload/request', body: uploadRequestData, requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
        print('✅ File Upload Request endpoint accessible (${response.statusCode})');
      } else {
  ...
  Future<void> testNotificationEndpoints() async {
    ...
    try {
      final response = await makeRequest('GET', '/api/v1/notifications/settings', requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
        print('✅ Notification Settings endpoint accessible (${response.statusCode})');
      } else {
  ...
  Future<void> testVerificationEndpoints() async {
    ...
    try {
      final response = await makeRequest('GET', '/api/v1/verification/requests', requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
        print('✅ Verification Requests endpoint accessible (${response.statusCode})');
      } else {
