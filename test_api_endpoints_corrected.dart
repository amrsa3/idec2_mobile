...
class ApiEndpointTester {
  ...
  Future<http.Response> makeRequest(
    ...
    try {
      ...
      if (response.body.isNotEmpty && response.body.length < 500) {
        ...
        print('📄 Response: [Large response - ${response.body.length} characters]');
      }
  ...
  Future<void> testServerHealth() async {
    ...
    try {
      final response = await makeRequest('GET', '/api/v1/health');
      if (response.statusCode == 200) {
        print('✅ Health endpoint working at /api/v1/health');
      } else {
        print('⚠️  Health endpoint at /api/v1/health returned: ${response.statusCode}');
      }
    } catch (e) {
      print('❌ Health endpoint at /api/v1/health failed: $e');
    }
  }
  ...
  Future<void> testAuthEndpoints() async {
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/auth/register', body: registerData);
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/auth/login', body: loginData);
      if (response.statusCode == 200) {
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/auth/check-user-status', body: loginData);
      if (response.statusCode == 200) {
    ...
    // Test refresh token
    try {
      final response = await makeRequest('POST', '/api/v1/auth/refresh', body: {'refreshToken': 'dummy-token'});
    ...
    // Test logout
    try {
      final response = await makeRequest('POST', '/api/v1/auth/logout', requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
  ...
  Future<void> testOtpEndpoints() async {
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/auth/request-otp', body: otpData);
      if (response.statusCode == 200) {
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/auth/verify-otp', body: verifyData);
    ...
    try {
      final response = await makeRequest('POST', '/api/v1/auth/resend-otp', body: resendData);
      if (response.statusCode == 200 || response.statusCode == 400) {
  ...
  Future<void> testUserEndpoints() async {
    ...
    try {
      final response = await makeRequest('GET', '/api/v1/auth/profile', requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
        print('✅ Get Auth Profile endpoint accessible (${response.statusCode})');
      } else {
    ...
    try {
      final response = await makeRequest('GET', '/api/v1/profiles/me', requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
        print('✅ Get User Profile endpoint accessible (${response.statusCode})');
      } else {
    ...
    try {
      final response = await makeRequest('PUT', '/api/v1/profiles/me', body: updateData, requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
        print('✅ Update User Profile endpoint accessible (${response.statusCode})');
      } else {
  ...
  Future<void> testFileEndpoints() async {
    ...
    // Test get files
    try {
      final response = await makeRequest('GET', '/api/v1/files?page=1&limit=10', requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
  ...
  Future<void> testNotificationEndpoints() async {
    ...
    try {
      final response = await makeRequest('GET', '/api/v1/notifications?page=1&limit=10', requiresAuth: true);
      if (response.statusCode == 200 || response.statusCode == 401) {
        print('✅ Get Notifications endpoint accessible (${response.statusCode})');
      } else {
  ...
  Future<void> runComprehensiveTests() async {
    ...
    print('- Status 401 = Authentication required (expected for protected endpoints)');
